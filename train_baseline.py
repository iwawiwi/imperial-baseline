...
from lipreading.imperial_baseline import Lipreading
...
def load_args(default_config=None):
    ...
    # -- directory
    parser.add_argument(
        "--data-dir", default="./data/lrw_small_roi", help="Loaded data directory"
    )
    parser.add_argument(
        "--label-path",
        type=str,
        default="./data/lrw_label/500WordsSortedList.txt",
...
def train(model, dset_loader, criterion, epoch, optimizer, logger):
    ...
    for batch_idx, data in enumerate(dset_loader):
        ...
        # TODO: For RESNET-TSM, we need to make sure that all mini-batches has the same batch-size, so we need to duplicate some samples 
...
def get_model_from_json():
    ...
    args.backbone_se = args_loaded.get("backbone_se", False) # if not present, default to False
    print("=> Using backbone with SE: {}".format(args.backbone_se))
    ...
    model = Lipreading(
        ...
        backbone_squeeze_excitation=args.backbone_se,
        n_batch=args.batch_size, # FIXME: This could be a problem when batch size is not specified (e.g. during inference time)
    ).cuda()
    calculateNorm2(model)
    # ==> print model summary
    summary(model, input_size=(args.batch_size, 1, 29, 88, 88), depth=3, device="cuda")
    return model
...
def main():
    ...
    if args.model_path:
        assert args.model_path.endswith(".pth") and os.path.isfile(
            args.model_path
