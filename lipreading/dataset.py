...
class MyDataset(object):
    def __init__(
        ...
        self._annotation_direc = annotation_direc
        ...
        if self.use_boundary or (self.is_var_length and data_partition == "train"):
            assert (
                self._annotation_direc is not None
            ), "Directory path provided for the sequence timestamp (--annotation-direc) should not be empty."
            assert os.path.isdir(
                self._annotation_direc
            ), f"Directory path provided for the sequence timestamp (--annotation-direc) does not exist. Directory input: {self._annotation_direc}"
    ...
    def _apply_variable_length_aug(self, filename, raw_data):
        ...
        info_txt = os.path.join(
            self._annotation_direc, *filename.split("/")[self.label_idx :]
        )  # swap base folder
    ...
    def __getitem__(self, idx):
        # ==> load data based on encoded pkl file
        # raw_data = self.load_data(self.list[idx][0])
        tensor = torch.load(self.list[idx][0])

        raw_data = tensor.get("video")
        raw_data = [jpeg.decode(img, pixel_format=TJPF_GRAY) for img in raw_data]
        # inp = np.stack(inp, 0) / 255.0 # FIXME: data is normalized into [0, 1], disable it for now, will be normalized in preprocessing
